...
public class CityApi {
    ...
    @PostMapping("/add")
    private City addNewCity(@Valid @RequestBody ViewDrCityDto viewDrCityDto){
        return cityRepository.save(new City(viewDrCityDto));
    }

    @DeleteMapping("/{city_id}")
    private City deleteCity(@PathVariable("city_id") Integer cityId){
        City city = cityRepository.deleteCityById(cityId);
        return facadeUtils.checkElemOnNull(city, cityId);
    }

    @PutMapping("/{city_id}")
    private City updateCity(@PathVariable("city_id") Integer cityId, @Valid @RequestBody ViewDrCityDto viewDrCityDto){
